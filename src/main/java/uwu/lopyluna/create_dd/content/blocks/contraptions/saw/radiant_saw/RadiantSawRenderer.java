...
public class RadiantSawRenderer extends SafeBlockEntityRenderer<RadiantSawBlockEntity> {

    public RadiantSawRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    protected void renderSafe(RadiantSawBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light,
                              int overlay) {
    ...
    protected void renderBlade(RadiantSawBlockEntity be, PoseStack ms, MultiBufferSource buffer, int light) {
        ...
        if (SawBlock.isHorizontal(blockState)) {
            if (speed > 0) {
                partial = DesiresPartialModels.RADIANT_SAW_BLADE_HORIZONTAL_ACTIVE;
            } else if (speed < 0) {
                partial = DesiresPartialModels.RADIANT_SAW_BLADE_HORIZONTAL_REVERSED;
            } else {
                partial = DesiresPartialModels.RADIANT_SAW_BLADE_HORIZONTAL_INACTIVE;
            }
        } else {
            if (speed > 0) {
                partial = DesiresPartialModels.RADIANT_SAW_BLADE_VERTICAL_ACTIVE;
            } else if (speed < 0) {
                partial = DesiresPartialModels.RADIANT_SAW_BLADE_VERTICAL_REVERSED;
            } else {
                partial = DesiresPartialModels.RADIANT_SAW_BLADE_VERTICAL_INACTIVE;
            }
    ...
    protected void renderShaft(RadiantSawBlockEntity be, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
        KineticBlockEntityRenderer.renderRotatingBuffer(be, getRotatedModel(be), ms,
                buffer.getBuffer(RenderType.solid()), light);
    }

    protected void renderItems(RadiantSawBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer,
                               int light, int overlay) {
        boolean processingMode = be.getBlockState()
                .getValue(RadiantSawBlock.FACING) == Direction.UP;
        if (processingMode && !be.inventory.isEmpty()) {
            boolean alongZ = !be.getBlockState()
                    .getValue(RadiantSawBlock.AXIS_ALONG_FIRST_COORDINATE);
            ms.pushPose();
    ...
    private static float getOffset(RadiantSawBlockEntity be, float partialTicks, boolean alongZ) {
    ...
    public static void renderInContraption(MovementContext context, VirtualRenderWorld renderWorld,
        ...
        Direction facing = state.getValue(RadiantSawBlock.FACING);
        ...
        if (RadiantSawBlock.isHorizontal(state)) {
            if (shouldAnimate)
                superBuffer = CachedBuffers.partial(DesiresPartialModels.RADIANT_SAW_BLADE_HORIZONTAL_ACTIVE, state);
            else
                superBuffer = CachedBuffers.partial(DesiresPartialModels.RADIANT_SAW_BLADE_HORIZONTAL_INACTIVE, state);
        } else {
            if (shouldAnimate)
                superBuffer = CachedBuffers.partial(DesiresPartialModels.RADIANT_SAW_BLADE_VERTICAL_ACTIVE, state);
            else
                superBuffer = CachedBuffers.partial(DesiresPartialModels.RADIANT_SAW_BLADE_VERTICAL_INACTIVE, state);
        }
        ...
        if (!RadiantSawBlock.isHorizontal(state)) {
            superBuffer.rotateZDegrees(state.getValue(RadiantSawBlock.AXIS_ALONG_FIRST_COORDINATE) ? 90 : 0);
        }
