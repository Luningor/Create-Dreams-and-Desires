...
import uwu.lopyluna.create_dd.content.blocks.contraptions.drill.shadow.ShadowDrillActorVisual;
import uwu.lopyluna.create_dd.content.blocks.contraptions.drill.shadow.ShadowDrillRenderer;

import javax.annotation.Nullable;
import java.util.Optional;

public class ShadowSawMovementBehaviour extends BlockBreakingMovementBehaviour {

    @Override
    public boolean isActive(MovementContext context) {
        return super.isActive(context)
                && !VecHelper.isVecPointingTowards(context.relativeMotion, context.state.getValue(ShadowSawBlock.FACING)
                .getOpposite());
    }

    @Override
    public Vec3 getActiveAreaOffset(MovementContext context) {
        return Vec3.atLowerCornerOf(context.state.getValue(ShadowSawBlock.FACING)
                        .getNormal())
                .scale(.65f);
    }
    ...
    public ActorVisual createVisual(VisualizationContext visualizationContext, VirtualRenderWorld simulationWorld, MovementContext movementContext) {
        return new ShadowDrillActorVisual(visualizationContext, simulationWorld, movementContext);
    }
    ...
    public boolean canBreak(Level world, BlockPos breakingPos, BlockState state) {
        return super.canBreak(world, breakingPos, state) && ShadowSawBlockEntity.isSawable(state);
    }
