...
import static uwu.lopyluna.create_dd.content.blocks.contraptions.drill.radiant.RadiantDrillBlockBreakingKineticBlockEntity.fortunedestroyBlock;

public class RadiantDrillMovementBehaviour extends BlockBreakingMovementBehaviour {
    ...
    public void tickBreaker(MovementContext context) {
        ...
        if (destroyProgress >= 10) {
            ...
            if (shouldDestroyStartBlock(stateToBreak))
                radiantDestroyBlock(context, breakingPos);
    ...
    public boolean isActive(MovementContext context) {
        return super.isActive(context)
                && !VecHelper.isVecPointingTowards(context.relativeMotion, context.state.getValue(RadiantDrillBlock.FACING)
                .getOpposite());
    }

    @Override
    public Vec3 getActiveAreaOffset(MovementContext context) {
        return Vec3.atLowerCornerOf(context.state.getValue(RadiantDrillBlock.FACING)
                .getNormal()).scale(.65f);
    }
    ...
    protected float getBlockBreakingSpeed(MovementContext context) {
        ...
        if (context.contraption instanceof CarriageContraption)
            lowerLimit = 2f;
        return Mth.clamp(Math.abs(context.getAnimationSpeed()) / 25f, lowerLimit, 16f);
    }

    @Override
    public boolean canBreak(Level world, BlockPos breakingPos, BlockState state) {
        return super.canBreak(world, breakingPos, state) && !state.getCollisionShape(world, breakingPos)
                .isEmpty() && !AllTags.AllBlockTags.TRACKS.matches(state) && !DesiresTags.AllBlockTags.RADIANT_DRILL_IMMUNE.matches(state);
    }
